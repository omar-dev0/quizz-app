...
abstract class DTOs{
  ...
  static List<ExamEntity> examsResponseByIdDto(ExamsBySubjectResponse examResponse){
    return examResponse.exams?.map((exam){
      return ExamEntity(exam.Id, exam.title, exam.duration, exam.subject, exam.numberOfQuestions);
    }).toList()??[];
  }
  ...
  static List<ExamQuestionsEntity> examQuestionsByExamIdDto(ExamQuestionsReponseModel response) {
    return response.questions?.map((item) {
      var answers = item.answers?.map((answer) => AnswersEntity(answer.answer, answer.key)).toList() ?? [];
      var exam = ExamEntity(
        item.exam?.Id,
        item.exam?.title,
        item.exam?.duration,
      ...
      return ExamQuestionsEntity(answers, item.type, item.Id, item.question, item.correct, exam);
    }).toList() ?? [];
  }
