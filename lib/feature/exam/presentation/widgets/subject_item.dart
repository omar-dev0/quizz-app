...
class SubjectItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<HomeViewModel, HomeScreenStates>(
      listener: (context,state){
        if(state is NavigateToSubjectExamsState){
          Navigator.of(context).push(MaterialPageRoute(builder: (_)=> const SubjectExamsScreen()));
        }
      },
      child: GestureDetector(
        onTap: () {
          homeViewModel.doAction(NavigateToSubjectExamsAction());
        },
        child: Container(
